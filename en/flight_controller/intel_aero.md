...
The Intel Aero comes with a preinstalled optical flow binary (OS version 1.6 or higher) which enables the Aero to stably fly based on vision. In order to use optical flow, a range sensor has to be installed first (see above).

To use the optical flow, just run
```
systemctl start aero-optical-flow
```
on the Aero. If you want to start the optical flow binary at boot, use
```
